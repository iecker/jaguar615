...
public class CargoServiceDAO extends PlcBaseJpaDAO {
	...
	public BigDecimal salarioPorCargo(PlcBaseContextVO context, String codigoCargo) {
		...
		String query 	= annotationPersistenceUtil.getNamedQueryByName(CargoView.class, "querySelByCodigo").query();
		
		CargoView cargo = null;
		
		try {
			cargo = (CargoView) apiCreateQuery(context, CargoView.class, query).setParameter("codigo",codigoCargo).getSingleResult();
