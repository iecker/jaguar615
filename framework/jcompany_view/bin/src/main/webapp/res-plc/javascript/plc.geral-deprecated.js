/**
 * plc.geral.deprecated
 * Funções javascripts utilizadas pelo jCompany/Jaguar 
 * As funções abaixo, estão "deprecated" e serão refatoradas em versões futuras.
 */

/**
 * Funções Utilizadas em "plc.geral.util.js"
 */
...
/**
 * Funções Utilizadas em "plc.geral.event.js"
 */



/**
 * Função para executar outras funções no onload da página
...
/** Inibe evento onclick */
function inibeOnClick (evt) {
	return false;
}


/** Funções Utilizadas em "plc.geral.impressao.js" */
...
/**
 * Funções Utilizadas em "plc.geral.popup.js"
 */
 ...
 * Os parâmetros devem ser passados para este função aos pares e na seguinte ordem: nome e valor do atributo.
...
/**
 * Funções Utilizadas em "plc.geral.tabs.js"
 */
...
function CheckNumerico(campo,tammax,teclapres) {
	...
	var caracteresEspeciais = new Array("!","@","#","$","%","^","&","*","(",")","_","-","+","=","{","}","|","[","]","\\","\"",":",";","'","<",">","?","/");
...
function redirecionaPesquisa(campos) {
	...
	for(var i=0; i<campo.length; i++) {
		...
		if (elemento==null) {
			...
			if (indice>0) {
				//url=url+"&"+campo[i]+"="+escape(elemento.options[indice].getAttribute('id'));
			...
			while(document.getElementById("corpo:formulario:"+campo[i]+":_"+indice) != undefined ) {
				if (document.getElementById("corpo:formulario:"+campo[i]+":_"+indice).checked) {
					url=url+"&"+campo[i]+"="+escape(document.getElementById("corpo:formulario:"+campo[i]+":_"+indice).getAttribute('id'));	
				} 
				indice = indice +1;
			}
...
/** Lógica de destaque autom?tico de campos focados */
			...
			if(evt.type == 'focus' || typeof oldClassName == "undefined" || oldClassName == null){
				if(className != "adicionaBorda"){
					...
					if(obj.type && type.indexOf("select") > -1)
						plc.jq(obj).addClass("adicionaFundo");
					else	
						plc.jq(obj).addClass("adicionaBorda");
				}	
			}else {
...
/**
 * Funções Utilizadas em "plc.geral.navigation.js"
 */
...
/** Lógicas gen?ricas na navegação autom?tica de tab folder */
...
/** Lógicas gen?ricas para troca de aba na navegação autom?tica de tab folder */
...
/** Executa navegação automática com pressionamento de seta para cima */
function navegaSetaParaCima(){
	...
	if (linhaSelecao != null && linhaSelecao != "undefined"){
		...
		numLinha = -1;
		/*getVarGlobal("trSelecao") == null || getVarGlobal("trSelecao") == "undefined" ? -1 : getVarGlobal("trSelecao").id.substring(21,getVarGlobal("trSelecao").id.lastIndexOf(":linhaSel"));*/
	}
...
/**
 * Funções Utilizadas em "plc.geral.integration.js"
 */

/**
* Grava resolução de vídeo do cliente no cookie.
* Essa informação é utilizada pelo jcpmonitor.
*/
