...
plc = {
		...
		if (configuracaoGrid.aoSelecionar){
			...
			aoSelecionarLinha = function(id){
				...
				if (modoJanelaPlc == 'modal' || modoJanelaPlc == 'popup') {
					...
					// gera a URL final					
					var redirectSemParams;
					
					if(redirect.indexOf("?") != -1){
						redirectSemParams = redirect.substring(0, redirect.indexOf('?'));
					} else{
						redirectSemParams = redirect;
					}
					
					document.location= redirectSemParams + (params ? ('?' + params + paramFwplc) : ('' +paramFwplc) );
	...
	,manipulaErroAjax: function( data ) {
		plc.mensagem(false);
		if (data.errorMessage && data.errorMessage.indexOf("WELD-000321")<0) {
			plc.mensagem("erro", data.errorMessage, false);
		} else if(data.errorMessage && data.errorMessage.indexOf("WELD-000321")>=0) {
			document.location = data.responseXML.URL.replace('cid', '');
		} else if (data.responseText && data.responseText.indexOf('j_security_check')>0)	 {
			document.location = data.responseXML.URL.replace(/cid=\d+/,'');
		}	
	}
};
plc.jq;