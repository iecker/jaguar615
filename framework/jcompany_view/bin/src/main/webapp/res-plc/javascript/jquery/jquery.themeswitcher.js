	(function (plc.jq) {
		...
		jquitr.addThemeRoller = function(){
			if(plc.jq('#inline_themeroller').size() > 0){
				plc.jq('#inline_themeroller').fadeIn();
			}
			else {
				plc.jq('<div id="inline_themeroller" style="display: none; position: fixed; background: #111; top: 25px; right: 25px; padding: 22px 0 15px 4px;width: 245px;height:400px; -webkit-border-radius: 6px; -moz-border-radius: 6px; z-index: 9999999;">'+
					...
					.draggable({
						start: function(){
							plc.jq('<div id="div_cover" />').appendTo('#inline_themeroller').css({width: plc.jq(this).width(), height: plc.jq(this).height(), position: 'absolute', top: 0, left:0});
						},
						stop: function(){
							plc.jq('#div_cover').remove();
						},
						opacity: 0.6,
						cursor: 'move'
					})
					.resizable({
						start: function(){
							plc.jq(this).find('iframe').hide();
						},
						stop: function(){
							plc.jq(this).find('iframe').show();
						},
						handles: 's'
					})
		...
		jquitr.closeThemeRoller = function () {
			plc.jq('#inline_themeroller').fadeOut();
		};
		...
		jquitr.reloadCSS = function(){
			...
			if(jquitr.trString !== currSrc && currSrc !== ''){
				...
				if( plc.jq("link[href*=parseTheme.css.php], link[href=ui.theme.css]").size() > 0){
					plc.jq("link[href*=parseTheme.css.php]:last, link[href=ui.theme.css]:last").eq(0).after(cssLink);
				} else {
					plc.jq("head").append(cssLink);
				}
				if( plc.jq("link[href*=parseTheme.css.php]").size() > 3){
					plc.jq("link[href*=parseTheme.css.php]:first").remove();
				}
			}
