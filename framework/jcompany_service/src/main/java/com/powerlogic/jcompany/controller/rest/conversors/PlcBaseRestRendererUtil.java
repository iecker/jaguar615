...
/**
 * Classe Base utilitária, que faz a transformação de Objetos e Entidades para
 * um formato HTML. Contém o algoritmo de escrita de objetos e entidades. Suas
 * implementações devem prover os métodos quem possuem as String's chaves para a
 * escrita.
 */
public abstract class PlcBaseRestRendererUtil implements IPlcRestRendererUtil {
	...
	public String createTextOutputWithPipes(List<?> lista, String propriedade) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		if (lista != null && !lista.isEmpty()) {
			...
			for (Object vo : lista) {
				...
				if (!isChaveNatura) {
					out.append((String) propertyUtilsBean.getProperty(vo, propriedade) + "|" + entityInstance.getId() + getLineBreakString());
				} else {
					Object idNaturalDinamico = entityInstance.getIdNaturalDinamico();

					out.append((String) propertyUtilsBean.getProperty(vo, propriedade));

					String[] propriedades = chavePrimaria.propriedades();
					for (String upProp : propriedades) {
						Object property = propertyUtilsBean.getProperty(idNaturalDinamico, upProp);
	...
	protected void buildCollection(StringBuilder out, Collection<?> collection, int deep) {
		...
		try{
			...
			log.debug("Erro ao serializar a colecao. Provavel que esteja mapeada como LAZY. Ignorando a colecao.");
		}
		finally{
			closeCollection(out);
		}
	...
	public String createObject(Object objeto) {
		setSelectedProperties((String[]) ((Map)objeto).get("plcSelProps"));
		((Map)objeto).remove("plcSelProps");
		StringBuilder out = new StringBuilder();
	...
	protected void buildObject(StringBuilder out, Object obj, int deep) {
		if (deep < 1 || obj == null) {
			...
			if (isEntity && ArrayUtils.isEmpty(getSelectedProperties())) {
			...
			for (PlcProjectionProperty property : getPropertyDescriptors(obj)) {
				...
				// Ignora propriedades nao pertencentes ao dominio e/ou proxiadas
				if (!isValidProperty(obj, isEntity, property.getName())) {
					continue;
				}
				// Obtem o valor via GET!
				Object value = null;
				try {
					value = propertyUtilsBean.getProperty(obj, property.toString());
				} catch (Exception e) {
					log.debug("Erro ao serializar o objeto.");
				}
	...
	private List<PlcProjectionProperty> getPropertyDescriptors(Object obj) {
		...
		for (PropertyDescriptor pd : propertyUtilsBean.getPropertyDescriptors(obj.getClass())) {
	...
	/**
	 * Verifica se um Objeto é de um tipo Java Básico (String, Numeros,
	...
	protected boolean isValidProperty(Object obj, boolean isEntity, String name) {
		if (propertyUtilsBean.isReadable(obj, name)) {
			if ("class".equalsIgnoreCase(name)) {
				return false;
			} else if (isEntity && ("idNaturalDinamico".equalsIgnoreCase(name) || "nomePropriedadePlc".equalsIgnoreCase(name) || "agregadosLazyPlc".equalsIgnoreCase(name) || "propsChaveNaturalPlc".equalsIgnoreCase(name) || "propsIdPlc".equalsIgnoreCase(name))) {
				return false;
			} else if (getSelectedProperties() != null && ArrayUtils.isNotEmpty(getSelectedProperties())) {
					return (ArrayUtils.contains(getSelectedProperties(), name));
			} else
				try {
					Object objeto = propertyUtilsBean.getProperty(obj, name);
					if(!isEntity(objeto) && objeto.getClass().getName().indexOf("$")>-1) {
						//objeto proxiado
						return false;
					}
				} catch (Exception e) {
					log.debug("Erro ao recuperar objeto. Provável que esteja mapeada como LAZY. Ignorando a coleção.");
				}
			return true;
		}
		return false;
	...
	/**
	 * Metodo auxiliar, utilizado após adição do ultimo elemento de uma lista ou
	 * ultima propriedade.
	 */
